...
cd /usr/local/src
wget -O csf.tgz https://download.configserver.com/csf.tgz --no-check-certificate
...
if [ -e ${PIGNORE} ]; then
	if ! grep -m1 -q '^user:mysql$' ${PIGNORE}; then
		echo "user:mysql" >> ${PIGNORE}
	fi
	if ! grep -m1 -q '^user:diradmin$' ${PIGNORE}; then
		echo "user:diradmin" >> ${PIGNORE}
	fi
fi
